...
# loading the model
lstm_ego_model = tf.keras.models.load_model("checkpoints/lstm_ttc_ego.hdf5", compile=False)

lstm_ego_model.compile(optimizer='adam', 
                 loss=model_loss, 
                 metrics=[euc_dist])
...
for test_idx in range(9800, len(ped_dataset)):
    ...
    predictions = lstm_ego_model.predict(test_data)[:,:,:2].reshape(-1, 2)
    end_time = time.time()
    ...
    if test_idx in situation_idxs:
        ...
        plt.savefig(f"images/ego_ttc/with_ttc/{test_idx}_lstm.png", bbox_inches='tight', pad_inches=0)
